...
const UPGRADES = [
  ...
  },
  {
    ...
    next:  s => `${(autoDelay({ autoSpeed: s.autoSpeed + 1 }) / 1000).toFixed(1)} 秒に1問になる`,
...
function submitAnswer(raw) {
  ...
  el.expressionCard.className = 'expression-card ' + (p.correct ? 'flash-ok' : 'flash-ng');
...
function autoActive() {
  return state.auto && state.autoOn;
}
...
function autoLoop() {
  ...
  const playing = round && round.index < round.problems.length;
  const wait = playing ? autoDelay(state) : AUTO_ROUND_MS;
...
function isScreen(name) {
  const map = { calc: el.screenCalc, result: el.screenResult,
                upgrade: el.screenUpgrade, prestige: el.screenPrestige };
  return !map[name].classList.contains('hidden');
}

// 'calc' / 'result' / 'upgrade' / 'prestige'
...
function renderGoal(r) {
  ...
  const pct = Math.max(0, Math.min(100, (best / GOAL_TOWER) * 100));
  el.goalFill.style.width = `${pct}%`;
  el.goalNote.innerHTML = state.stats.goal
    ? `到達済み　自己最高 <b>${towerStr(best)}</b>`
    : `今回 <b>${towerStr(now)}</b> ／ 自己最高 <b>${towerStr(best)}</b> ／ 到達度 <b>${pct.toFixed(2)}%</b>`;
  el.goalBadge.classList.toggle('hidden', !(r && r.justReached));
}
...
function renderPrestige() {
  ...
  const from = Math.pow(gain, 2) * SIGMA_DIV;
...
function renderStats() {
  ...
  const rows = [
    ...
    ['最終目標への到達度', state.stats.goal
      ? '達成！'
      : `${Math.max(0, state.stats.bestTower / GOAL_TOWER * 100).toFixed(2)}%（${towerStr(state.stats.bestTower)}）`],
...
function renderAll() {
  el.points.textContent = fmt(state.points);
  el.idleProblems.textContent = state.problems;
  renderUpgrades();
  renderOps();
  renderAuto();
  renderAllIn();
